...
function getSystemInfo() {
  ...
  return { system, cpu, arch, uptime, ramUsage, ramTotal, ramUsagePercent };
}
function printLog({
  system,
  arch,
  ...
  console.log(`Modelo de processador: ${cpu[0].model}`);
...
function saveLog({
  ...
  const logContent = `DETALHES DO SISTEMA | Sistema Operacional: ${system} | Arquitetura: ${arch} | Modelo do Processador: ${cpu[0].model} | Tempo de Atividade do Sistema: ${uptime} | Uso de Memória RAM: ${ramUsage.toFixed(2)} GB / ${ramTotal.toFixed(2)} GB (${ramUsagePercent} %)\n---\n`;
